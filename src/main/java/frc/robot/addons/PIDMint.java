...
// MINT Additions 2025 Automatons.
...
public class PIDMint implements Sendable, AutoCloseable {
   ...
   */
  public PIDMint(double kp, double ki, double kd, double tolerance, double min) {
    this(kp, ki, kd, 0.02, tolerance, min);
  }
  ...
  public PIDMint(double kp, double ki, double kd, double period, double tolerance, double min) {
    m_kp = kp;
    m_ki = ki;
    m_kd = kd;
    min_output = min;
  ...
  public double calculate(double measurement) {
    ...
    if (Math.abs(m_error) < m_errorTolerance) return 0;
    double result = m_kp * m_error + m_ki * m_totalError + m_kd * m_errorDerivative;
    if (Math.abs(result) < min_output) result = Math.signum(result) * min_output;
    return result;
  }
